...
require("lazy").setup({
	...
	-- AI 代碼補全（使用 neocodeium 替代原 codeium.nvim，解決補全衝突）
	{
		...
		config = function()
			...
			end, { desc = "Trigger Codeium completion" })
		end,
	},
	-- AI 對話與代碼解釋
	{
	...
	-- 深度學習／AI 模型集成（支持 OpenAI 和本地模型）
	{
		"dense-analysis/neural",
		cmd = { "Neural", "NeuralChat", "NeuralExplain" },
		keys = {
			{ "<leader>na", "<cmd>Neural<CR>", desc = "Neural AI" },
			{ "<leader>ne", "<cmd>NeuralExplain<CR>", desc = "Neural Explain" },
		},
		config = function()
			vim.g.neural = {
				source = "openai",
...
vim.opt.autoread = true -- 自動重新讀取被外部修改的文件，解決 :wq 需要 ! 的問題
